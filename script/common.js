...
function stupid(){
 setTimeout(() => {
    DisplErr("You're a certified Moron!");
 }, 1 * 1000);
 window.location.href ='../Html/about_auth.php';
}
...
function UserOpts() {
   ...
   if (GetCookStat !== false) {
      ...
      console.error("You are not signed in right now. Sign in with your Sammlung account to access this feature.");
   }
}
...
function LogOut() {
   ...
   if (arguments.length > 0) {
      ...
      DisplErr("You are signed out!");

      setTimeout(()=>{
         DisplErr("You will be Redirected!");
      },1 * 1000);
...
function ReqDel(event, type) {
   ...
   if (type === 1 || type === '1') {
      ...
      alert(a12.length +" "+b12.length);
      if ((a12.length1) < 4 || (b12.length)< 8) {
        ...
        if (xhr.status === 200) {
            ...
            if (response.status === 'success') {
               // alert(response.message);
                // Further actions on successful upload
                DisplErr('Upload Successful');
               console.log('Upload successful', response.title, response.fileName);
            } else {
             //   alert(response.message);
                // Handle upload errors
                DisplErr(response.message);
                console.log('Upload error: ', response.message);
            }
        } else {
            DisplErr('Upload failed');
